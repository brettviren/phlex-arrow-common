// Unit test for Arrow-product selection (ddm-c3s.2 #2).
...
int main()
{
    ...
    store.add_product<arrow_table_ptr>(pe::product_specification("frame"), arrow_table_ptr{});
    store.add_product<arrow_table_ptr>(pe::product_specification("depos"), arrow_table_ptr{});
    store.add_product<int>(pe::product_specification("count"), 5);

    auto sel = select_arrow_products(store);
    check(sel.size() == 2, "two Arrow products selected (not the int)");
    ...
    check(is_arrow_product(typeid(arrow_table_ptr)), "is_arrow_product true for arrow_table_ptr");
